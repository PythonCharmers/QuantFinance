...
def plot_kmeans_interactive(min_clusters=1, max_clusters=6):
    ...
    with warnings.catch_warnings():
        ...
        def _kmeans_step(frame=0, n_clusters=4):
            ...
            plt.scatter(
                old_centers[:, 0], old_centers[:, 1], marker="o", c="black", s=50
            )
    ...
    return interact(
        _kmeans_step, frame=[0, 50], n_clusters=[min_clusters, max_clusters]
    )
...
def plot_image_components(
    ...
    for i in range(0, n_components):
        ...
        show(1, i + 3, approx, r"${0:.2f} \cdot c_{1}$".format(coefficients[i], i + 1))
        plt.gca().text(
            0,
            1.05,
            "$+$",
